...
namespace DotNet.Utilities
{
    ...
    public class BaseBusinessLogic
    {
        ...
        public static string FieldCreateDate = "CreateDate";        // 创建时间
        public static string FieldModifyUserId = "ModifyUserId";    // 最后修改者主键
        public static string FieldModifyDate = "ModifyDate";        // 最后修改时间
        ...
        public static string GetAuditStatus(string auditStatus)
        {
            ...
            if (AuditStatus.StartAudit.ToString().Equals(auditStatus))
            {
                returnValue = "递交审核";
            }
        ...
        public static PermissionScope GetPermissionScope(string[] organizeIds)
        {
            ...
            {
                if (BaseBusinessLogic.Exists(organizeIds, permissionScope.ToString()))
                {
        ...
        #region public static byte[] GetBinaryFormatData(DataTable dataTable) 服务器上面取数据,填充数据集,转换为二进制格式.
        /// <summary>
        ...
        #region public static DataTable RetrieveDataSet(byte[] ArrayResult) 客户端接收到byte[]格式的数据,对其进行反序列化,得到数据集,进行客户端操作.
        /// <summary>
        /// 客户端接收到byte[]格式的数据,对其进行反序列化,得到数据集,进行客户端操作.
        /// </summary>
        ...
        #region public static DataTable SetFilter(DataTable dataTable, string name, string value) 对数据表进行过滤
        /// <summary>
        /// 对数据表进行过滤
        /// </summary>
        /// <param name="dataTable">数据表</param>
        ...
        public static DataTable SetFilter(DataTable dataTable, string name, string value, bool equals)
        {
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                if (equals)
                {
                    if (dataRow[name].ToString().Equals(value))
                    {
                        dataRow.Delete();
                    }
                }
        ...
        #region public static DataRow GetDataRow(DataTable dataTable, string id) 从数据集读取一行数据
        /// <summary>
        /// 从数据集读取一行数据
        /// </summary>        
        /// <param name="dataTable">数据表</param>
        ...
        #region public static DataRow GetDataRow(DataTable dataTable, string fieldName, string fieldValue) 从数据集读取一行数据
        /// <summary>
        /// 从数据集读取一行数据
        /// </summary>        
        /// <param name="dataTable">数据表</param>
        ...
        public static int EndDebug(MethodBase methodBase, int milliStart)
        {
            ...
            #if (DEBUG)
                Console.WriteLine(DateTime.Now.ToString(BaseSystemInfo.DateTimeFormat) + " Ticks: " + TimeSpan.FromMilliseconds(milliEnd - milliStart).ToString() + " :End: " + methodBase.ReflectedType.Name + "." + methodBase.Name);
                Trace.WriteLine(DateTime.Now.ToString(BaseSystemInfo.DateTimeFormat) + " Ticks: " + TimeSpan.FromMilliseconds(milliEnd - milliStart).ToString() + " :End: " + methodBase.ReflectedType.Name + "." + methodBase.Name);
            #endif
